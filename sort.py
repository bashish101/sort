...
class SortNet(nn.Module):
	def __init__(self, input_size, hidden_size):
		...
		self.conv1 = nn.Conv1d(1, 1, 1, bias = None)
		self.fc2 = nn.Linear(hidden_size, input_size, bias = None)
...
def train(batch_size, count, max_val, save_path, resume_flag = False):
	...
	input_size = count
	hidden_size = sum(range(input_size + 1))	
...
def test(batch_size, count, max_val, save_path):
	...
	for inp, tgt, out in zip(np.array(x), np.array(y), np.array(pred.detach())):
		out = [np.clip(round(idx), 0, count) for idx in out]
...
if __name__  == '__main__':
	...
	if mode == 'train':
		train(batch_size, count, max_val, save_path, resume_flag = True)
	else:
